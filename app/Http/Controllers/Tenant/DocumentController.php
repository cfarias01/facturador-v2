...
use App\Http\Resources\Tenant\DocumentosDevueltosCollection;
use App\Http\Resources\Tenant\DocumentosEmitidosCollection;
...
class DocumentController extends Controller
{
    ...
    public function index_failed()
    {
        ...
        return view('tenant.documents_failed.index', compact(
            'is_client',
            'import_documents',
            'import_documents_second',
            'document_import_excel',
    ...
    public function index_returned()
    {
        $is_client = $this->getIsClient();
        $import_documents = config('tenant.import_documents');
        $import_documents_second = config('tenant.import_documents_second_format');
        ...
        return view('tenant.documents_returned.index', compact(
            'is_client',
            'import_documents',
            'import_documents_second',
            'document_import_excel',
    ...
    public function records(Request $request)
    {
        try {
            ...
            return new DocumentosEmitidosCollection($records->paginate(10));

        } catch (Exception $e) {
            Log::error('Error en el controlador de documentos: ' . $e->getMessage());
        }
    }

    public function getdocumentsReturned(Request $request)
    {

        try {
            $records = CabeceraDocumentoElectronica::whereNotIn('idEstado', ['05', '01','11']);
            return new DocumentosDevueltosCollection($records->paginate(config('tenant.items_per_page')));
        } catch (Exception $e) {
            Log::error('Error en el controlador de getdocumentsReturned: ' . $e->getMessage());
        }
        
    }

    public function reSendDocument($idInterno)
    {
        try {
            $document = CabeceraDocumentoElectronica::where('idInterno', $idInterno)->first();
            $document->idEstado = '01';
            $document->save();
            ...
            return response()->json([
                'success' => false,
                'message' => 'Error al reporcesar el documento: ' . $e->getMessage()
            ]);
        }
    }

    public function reCrateDocument($idInterno)
    {
		
        try {

            $document = CabeceraDocumentoElectronica::where('idComporbante', $idInterno)->first();

            Log::error('Reprocesando documento: ' . $idInterno. ' - ' . $document->claveAcceso);

            $servicio = new IntegradorService();
            $company = Company::first();

            $servicio->deleteDocumentClaveAcceso($company, $document->claveAcceso);
    ...
    public function reCreateDocumentAll(){

        try {

            $documents = CabeceraDocumentoElectronica::whereIn('idEstado',['30'])->get();
            $servicio = new IntegradorService();
            $company = Company::first();
            ...
            return response()->json([
                'success' => true,
                'message' => 'Todos los documentos se han actualizado correctamente'
            ]);

        } catch (Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Error al reporcesar los documentos: ' . $e->getMessage()
            ]);
        }

    }

    public function getDocumentsFailed(Request $request)
    {

        $serie = $request->input('serie') ?? null;
        $numero = $request->input('numero') ?? null;
        $fechaIni = $request->input('fechaIni') ?? null;
        $fechaFin = $request->input('fechaFin') ?? null;

        $servicio = new IntegradorService();
        $company = Company::first();

        $data = $servicio->getDocumentsFailed($company, $serie, $numero, $fechaIni, $fechaFin);
        return $data;
    }

    public function processdoccumentsFailed(Request $request)
    {
        ...
        $data = $servicio->reprocesarDocumentosFailed($company, $serie, $numero, $fechaIni, $fechaFin);
        return $data;
    }
    ...
    public function table($table)
    {
        ...
        if ($table === 'payment_method_types') {
            ...
            return NULL; //PaymentMethodType::getPaymentMethodTypes();
            /*
    ...
    {
        $record = new DocumentResource(CabeceraDocumentoElectronica::findOrFail($id));
    ...
    public function email(DocumentEmailRequest $request)
    {
        ...
        $email = '';
        if(strpos($documento->correo, '|')){
            $email = explode('|',$documento->correo);
            $email = trim($email[0]);
        }else{
    ...
    public function getRecords($request)
    {
        ...
        $records = CabeceraDocumentoElectronica::query();
        ...
        $docuemntoFiltor = $request->nameDocument;

        if(!$date_of_issue && !$d_start && !$d_end ){
            $records->whereDate('fecha', '>=', date("Y-m-d"));
        }

        if ($date_of_issue) {

            $records->where('fecha', '=',$date_of_issue);
        }  

        if ($state_type_id) {
            // if multiple estados are provided like "01,05" -> whereIn
            $state_type_id = trim($state_type_id);
            if (strpos($state_type_id, ',') !== false) {
                $states = array_map('trim', explode(',', $state_type_id));
                $records->whereIn('idEstado', $states);
            } else {
                $records->where('idEstado', $state_type_id);
            }
        }

        if ($document_type_id) {
            // Normalize document_type_id like "01" -> "1" (tipoComprobante stored as single char)
            $docDigits = preg_replace('/\D/', '', $document_type_id);
            $docChar = substr($docDigits, -1);
            if ($docChar !== '') {
                $records->where('tipoComprobante', $docChar);
            }
        }
        if ($number) {
            $records->where('orderNo', $number);
        }
        if ($d_start && $d_end) {
            // normalize dates to avoid DB functions and ensure proper format
            try {
                $start = \Carbon\Carbon::parse($d_start)->format('Y-m-d');
                $end = \Carbon\Carbon::parse($d_end)->format('Y-m-d');
                $records->whereBetween('fecha', [$start, $end]);
            } catch (\Exception $e) {
                // fallback if parse fails
                $records->whereBetween('fecha', [$d_start, $d_end]);
            }
        }
        if ($customer_id) {
            $records->where('ruc', '=', $customer_id );
        }
        if ($customer_name) {

            $records->where('cliente', 'like', '%' . $customer_name . '%');
        }
        if ($docuemntoFiltor) {

            $records->where('idInterno', 'like', '%' . trim($docuemntoFiltor) . '%');
            // avoid DB function so index on fecha can be used: use PHP to compute first day of current month
            $firstDayOfMonth = date('Y-m-01');
            $records->where('fecha', '>=', $firstDayOfMonth);
        }

        $records->orderBy('fecha', 'DESC');
    ...
    public function getRecords2($request)
    {
        ...
        if ($date_of_issue) {
            ...
            $records->where('fecha', '=',$date_of_issue)->orderBy('orderNo', 'DESC');
        }  

        if ($state_type_id) {
            $records->where('idEstado', 'like', '%' . $state_type_id . '%');
        }
        if ($document_type_id) {
            $records->where('tipoComprobante', 'like', '%' . substr($document_type_id, 1, 1) . '%');
        }
        if ($number) {
            $records->where('orderNo', $number);
        }
        ...
        if ($docuemntoFiltor) {
            ...
            $records->Where('idInterno', 'like', '%' . trim($docuemntoFiltor) . '%');
            $records->where('fecha','>=', DB::raw('DATE_SUB(CURDATE(), INTERVAL DAY(CURDATE()) - 1 DAY)')); // Filter records from the last year
        } 
    ...
    public function data_table()
    {
        ...
        return compact('customers', 'document_types', 'series', 'establishments', 'state_types', 'items');
    }
    ...
    public function report_payments(Request $request)
    {
        ...
        $date_of_issue = $request->date_of_issue;
        $state_type_id = $request->state_type_id;
        $document_type_id = intval($request->document_type_id);
        $number = $request->number;
        $d_end = $request->d_end;
        $d_start = $request->d_start;
        $customer_id = $request->customer_id;
        ...
        if ($date_of_issue) {
            ...
            $records->where('fecha', 'like', '%' . $date_of_issue . '%');
        }
        if ($state_type_id) {
            $records->where('idEstado', 'like', '%' . $state_type_id . '%');
        }
        if ($document_type_id > 0) {
            Log::info("ID TIPO : " . $documentTypeId);
            $records->where('tipoComprobante', 'like', '%' . $document_type_id . '%');
        }
        if ($number) {
            $records->where('orderNo', $number);
        }
        ...
        if ($customer_id) {
            $records->where('ruc', 'like', '%' . $customer_id . '%');
        }
        if ($customer_name) {
        ...
        if ($docuemntoFiltor) {
            ...
            $records->where('nombreDoc', 'like', '%' . trim($docuemntoFiltor) . '%')->orWhere('idInterno', 'like', '%' . trim($docuemntoFiltor) . '%');
        } else {
        ...
        foreach ($records as $item) {
            ...
            $tipoComprobante = '0' . $item->tipoComprobante;

            $estado = StateType::find($idEstado)->description;
            $documentTD = DocumentType::find($tipoComprobante)->description;

            $item->descrpIdEstado = $estado;
            $item->descrpTipoComprobante = $documentTD;
        }
        ...
        //Log::info("NUMERO DE RECORD: ".count($records));
    ...
    public function destroyDocument($document_id)
    {
        try {
            ...
            DB::connection('tenant')->transaction(function () use ($document_id) {
                ...
                $company = Company::first();
                $record = CabeceraDocumentoElectronica::findOrFail($document_id);
                $serie = $record->idInterno;
                $serie = explode('-', $serie);

                $servicio = new IntegradorService();
                $servicio->deleteDocument($company, $serie[0], $serie[1]);
                $record->delete();
            });
    ...
    public function storeBrands($request)
    {
