...
class Template
{
    public function pdf($base_template, $template, $company, $document, $details, $returnPurchases, $format_pdf)
    {
        if($template === 'credit') {
            $template = 'note';
        }
        ...
        return self::render($path_template, $company, $document, $details, $returnPurchases);
    }
    ...
    private function render($view, $company, $document, $details, $returnPurchases)
    {
        view()->addLocation(app_path('CoreFacturalo'.DIRECTORY_SEPARATOR.'Templates'));
        ...
        if (view()->exists($view))
        {
            return view($view, compact('company', 'document', 'details', 'returnPurchases'))->render();

        }else{
            Log::info("NO SE ENCONTRO LA VISTA: ".app_path('CoreFacturalo'.DIRECTORY_SEPARATOR.'Templates'));
        }
    ...
    public function validate_template($base_template, $template, $format_pdf)
    {
        ...
        if(file_exists($path_app_template.DIRECTORY_SEPARATOR.$path_template.'.blade.php')) {
            //Log::info('validate_template: '.str_replace(DIRECTORY_SEPARATOR, '.', $path_template));
            return str_replace(DIRECTORY_SEPARATOR, '.', $path_template);
            //return str_replace(DIRECTORY_SEPARATOR, '.','CoreFacturalo'.DIRECTORY_SEPARATOR.'Templates'.DIRECTORY_SEPARATOR.$path_template.'.blade.php');
        }
    ...
    public function pdfWithoutFormat($base_template, $template, $company, $document)
    {
        ...
        // Pass empty arrays for $details and $returnPurchases as defaults
