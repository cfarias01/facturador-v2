...
use App\Models\System\PaymentMethodType as SystemPaymentMethodType;
...
class DocumentPaymentController extends Controller
{
    ...
    {
        $records = CabeceraDocumentoElectronica::find($document_id)->fPagos;
        $records = json_decode($records,true);

        $collection = null;
        foreach($records as $pago){

            $descripcionEstado =SriFormasPagos::where('code',$pago[0])->get();
    ...
    {
        return [
            'payment_method_types' => SystemPaymentMethodType::all(),
            'payment_destinations' => $this->getPaymentDestinations(),
            'permissions' => null //auth()->user()->getPermissionsPayment()
        ];
    }

    public function document($document_id)
    {
        $document = CabeceraDocumentoElectronica::find($document_id);

        if ($document->retention) {
            $total = $document->total - $document->retention->amount;
        } else {
            $total = $document->importeTotal;
        }

        $FormasPago = $document->fPagos;
        $total_paid = 0;
        $FormasPago = json_decode($FormasPago,true);

        foreach($FormasPago as $pago){
            $total_paid = $total_paid + $pago[1];
        }
        ...
        return [
            'number_full' => $document->idComporbante,
