...
class ClientController extends Controller
{
    ...
    public function tables()
    {
        ...
        $group_facturador = Module::with('levels')
            ->whereIn('id', [1, 2, 7])
            ->orderBy('sort')
            ->get()
            ->each(function ($module) {
    ...
    public function records()
    {
        ...
        foreach ($records as &$row) {
            ...
            /*$row->count_doc = DB::connection('tenant')
                    ...
                    ->first()
                    ->quantity_documents;*/
            $row->count_doc = DB::connection('tenant')->table('cabecera_documento_electronicas')->count();
    ...
    public function record($id)
    {
        ...
        $client->ftpEmitidosServer = $company->ftpEmitidosServer;
        $client->ftpEmitidosPort = $company->ftpEmitidosPort;
        $client->ftpEmitidosUser = $company->ftpEmitidosUser;
        $client->ftpEmitidosPass = $company->ftpEmitidosPass;
        $client->ftpEmitidosRutaFac = $company->ftpEmitidosRutaFac;
        $client->ftpEmitidosRutaNota = $company->ftpEmitidosRutaNota;
        $client->ftpEmitidosRutaRetencion = $company->ftpEmitidosRutaRetencion;
        $client->emailLogo = $company->emailLogo;
            ...
            for ($i = 1; $i <= 12; $i++) {
                ...
                $count_documents[] = [
                    ...
                    'count' => $row->count_doc = DB::connection('tenant')
                        ...
                        ->whereBetween('fecha', [$date_initial, $date_final])
                        ->count()
                ];
            }
        }
    ...
    public function store(ClientRequest $request)
    {
        ...
        DB::connection('tenant')->table('companies')->insert([
            ...
            'ftpEmitidosServer' => $request->input('ftpEmitidosServer'),
            'ftpEmitidosPort' => $request->input('ftpEmitidosPort'),
            'ftpEmitidosUser' => $request->input('ftpEmitidosUser'),
            'ftpEmitidosPass' => $request->input('ftpEmitidosPass'),
            'ftpEmitidosRutaFac' => $request->input('ftpEmitidosRutaFac'),
            'ftpEmitidosRutaNota' => $request->input('ftpEmitidosRutaNota'),
            'ftpEmitidosRutaRetencion' => $request->input('ftpEmitidosRutaRetencion'),
            'emailLogo' => $request->input('emailLogo'),
