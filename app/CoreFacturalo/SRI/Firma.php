...
class Firma
{
    ...
    {
        //Log::info("Iniciando verificaci      n de certificado PKCS#12", ['file' => $this->config['file']]);

        try {
            $pathFirma = $this->config['file'];

            if (!file_exists($pathFirma)) {
                //Log::error("Certificado no encontrado", ['path' => $pathFirma]);
                return ['error' => true, 'mensaje' => "El archivo del certificado no existe: $pathFirma"];
            }

            if (!is_readable($pathFirma)) {
                //Log::error("Certificado no legible", ['path' => $pathFirma]);
                return ['error' => true, 'mensaje' => "El archivo del certificado no es legible: $pathFirma"];
            }

            $contenido = file_get_contents($pathFirma);

            if ($contenido === false || strlen($contenido) === 0) {
                //Log::error("Contenido vac      o o ilegible del archivo", ['path' => $pathFirma]);
                return ['error' => true, 'mensaje' => "No se pudo leer el contenido del archivo: $pathFirma"];
            }

            $certs = [];
            if (!openssl_pkcs12_read($contenido, $certs, $this->config['pass'])) {
                $opensslErr = '';
                while ($msg = openssl_error_string()) {
                    $opensslErr .= $msg . " | ";
                }
                Log::error("Fallo al leer archivo PKCS#12", [
                    'archivo' => $pathFirma,
                    'password' => $this->config['pass'],
                    'openssl_error' => $opensslErr
                ]);
                return ['error' => true, 'mensaje' => "No se puede leer contenido del archivo p12. Puede que la contrase      a est       incorrecta o el archivo est       corrupto."];
            }

            if (!isset($certs['cert'])) {
                ////Log::error("No se encontr       el certificado principal", ['certs' => $certs]);
                return ['error' => true, 'mensaje' => "No se encontr       el certificado principal en el archivo p12"];
            }

            $x509cert = openssl_x509_read($certs['cert']);
            if (!$x509cert) {
                ////Log::error("openssl_x509_read fall       con el certificado principal.");
                return ['error' => true, 'mensaje' => "No se pudo interpretar el certificado X.509"];
            }

            $certf = openssl_x509_parse($x509cert);
            if (!$certf) {
                //Log::error("openssl_x509_parse fall       al intentar parsear el certificado.");
                return ['error' => true, 'mensaje' => "No se pudo parsear la informaci      n del certificado"];
            }

            $subject = $certf['subject']['CN'] ?? '(CN no definido)';
            $certificante = $certf['subject']['O'] ?? null;
            $aux = null;

            if ($certificante === "BANCO CENTRAL DEL ECUADOR") {
                $aux = $certs['extracerts'] ?? [];
                foreach ($aux as $item) {
                    $x509certExtra = openssl_x509_read($item);
                    $certDataExtra = openssl_x509_parse($x509certExtra);

                    $this->certificate = $x509certExtra;
                    $this->certData = $certDataExtra;
                    break;
                }
            } else {
            ...
            if (!$this->certificate) {
                //Log::error("No se pudo cargar un certificado v      lido del archivo.");
                return ['error' => true, 'mensaje' => "No se pudo cargar un certificado v      lido del archivo"];
            }

            $this->publicKey = openssl_get_publickey($this->certificate);
            if (!$this->publicKey) {
                //Log::error("No se pudo extraer la clave p      blica del certificado.");
                return ['error' => true, 'mensaje' => "No se pudo extraer la clave p      blica del certificado"];
            }

            if ($this->getPublicPem() === "") {
                //Log::error("El contenido PEM de la clave p      blica est       vac      o.");
                return ['error' => true, 'mensaje' => "No se pudo obtener el contenido PEM de la clave p      blica"];
            }

            $resp = $this->getPrivateKey();
            if ($resp["error"] === true) {
                //Log::error("Error al obtener clave privada", ['mensaje' => $resp["mensaje"]]);
                return ['error' => true, 'mensaje' => "Error al obtener clave privada: " . $resp["mensaje"]];
            }
            $fecha_actual = strtotime(date("Y-m-d H:i:s"));
            $fecha_entrada = strtotime(date("Y-m-d H:i:s", $this->certData['validTo_time_t']));
            if ($fecha_actual > $fecha_entrada) {
                Log::error("El certificado ha expirado", [
                    'fecha_actual' => $fecha_actual,
                    'fecha_expiracion' => $fecha_entrada
                ]);
                return ['error' => true, 'mensaje' => "El certificado ha expirado. Actualice su certificado con la Autoridad Certificadora."];
            }

            if ($aux != null) {
                if (($rucCert = $this->getRucCert($aux)) === false) {
                    Log::error("No se pudo extraer el RUC del certificado.");
                    return ['error' => true, 'mensaje' => "No se pudo extraer el RUC del certificado"];
                }

                $rucCmp = substr($this->claveAcceso, 10, 13);
                if ($rucCmp !== $rucCert) {
                    Log::error("El RUC del certificado no coincide con el del emisor", [
                        'ruc_certificado' => $rucCert,
                        'ruc_emisor' => $rucCmp
                    ]);
                    return ['error' => true, 'mensaje' => 'El RUC del certificado no coincide con el del emisor'];
                }
            }

            $this->generarId();
        } catch (Exception $ex) {
            Log::error("Excepci      n capturada en verificarCertPKey: " . $ex->getMessage(), ['trace' => $ex->getTraceAsString()]);
            return ['error' => true, 'mensaje' => "Excepci      n atrapada: " . $ex->getMessage()];
        }

        //Log::info("Certificado PKCS#12 verificado correctamente", ['file' => $this->config['file']]);
        return ['error' => false, 'mensaje' => ""];
    }

    public function verificarCertPKey_old()
    {
        ...
        try {
            ...
            $contenido = file_get_contents($pathFirma, FILE_USE_INCLUDE_PATH);
            if ($contenido === false) {
                Log::error("No se pudo leer el archivo: $pathFirma\n");
            }
    ...
    private function getPrivateKey()
    {
        try {
            ...
            $password = $this->config['pass'];
            $nombreKey = "firma_temp3.pem";

            if (file_exists($nombreKey))
                unlink($nombreKey);

            $aux = 'C:\OpenSSL-Win64\bin\openssl.exe pkcs12 -in ' . $pfx . ' -nocerts -out ' . $nombreKey . ' -passin pass:' . $password . ' -passout pass:' . $password . ' 2>&1';
            //Log::info("Ejecutando openssl con: /usr/bin/openssl pkcs12 -in " . $pfx . " -nocerts -out " . $nombreKey . ' -passin pass:' . $password . ' -passout pass:' . $password . ' 2>&1');
         
            if (str_contains(strtoupper(php_uname("s")), "WINDOWS")) {
                $salida = shell_exec('C:\OpenSSL-Win64\bin\openssl.exe pkcs12 -in ' . $pfx . ' -nocerts -nodes -out ' . $nombreKey . ' -passin pass:' . $password . ' -passout pass:' . $password . ' 2>&1');
            }else{
                $salida = shell_exec('sudo /usr/bin/openssl pkcs12 -in ' . $pfx . ' -nocerts -nodes -out ' . $nombreKey . ' -passin pass:' . $password . ' -passout pass:' . $password . ' 2>&1');
                Log::info($salida);
            }

            if (file_exists($nombreKey) && filesize($nombreKey) > 0) {
    ...
    private function getPrivateKey_old()
    {
        try {
            ...
            //Log::info($salida);
            //Log::info(php_uname("s"));

            if (str_contains(strtoupper(php_uname("s")), "WINDOWS")) {
                //ejecutar comando openssl en windows//
                $salida = shell_exec('C:\OpenSSL-Win64\bin\openssl.exe pkcs12 -in ' . $pfx . ' -nocerts -out ' . $nombreKey . ' -passin pass:' . $password . ' -passout pass:' . $password . ' 2>&1');
            }
    ...
    public function firmar($xml)
    {
        ...
        try {
            ...
            if ($signatureResult != null) {
                ...
                if ($this->tipoComprobante === '04')
                    $xml = str_replace('</notaCredito>', $sig . '</notaCredito>', $xml);
                if ($this->tipoComprobante === '05')
                    $xml = str_replace('</notaDebito>', $sig . '</notaDebito>', $xml);
                if ($this->tipoComprobante === '06')

                    //Log::info("firma generada para guia");
                    //Log::info($sig);

                    $xml = str_replace('</guiaRemision>', $sig . '</guiaRemision>', $xml);

                if ($this->tipoComprobante === '03')
                    $xml = str_replace('</liquidacionCompra>', $sig . '</liquidacionCompra>', $xml);
