@php
    ...
    foreach($details as $item){
        ...
        $BASE = (floatVal($item['precioUnitario']) * floatVal($item['cantidad'])) - floatVal($item['descuento']);
        $valor = ($BASE * $item['iva']) /100;

        foreach ($totales as $key => $value) {
            if($value['tarifa'] == intVal($item['iva'])){
                $existe = true;
                $totales[$key]['iva'] += $valor;
                $totales[$key]['subtotal'] += $BASE;
            }
        }
                ...
                <td width="50%">
                    @if($company->logo)
                            ...
                            <div class="company_logo_box">
                                <img src="data:{{mime_content_type(public_path("storage/uploads/logos/{$company->logo}"))}};base64, {{base64_encode(file_get_contents(public_path("storage/uploads/logos/{$company->logo}")))}}" alt="{{$company->name}}" class="company_logo" style="margin-left: 50px; padding-bottom: 40px; max-width: 200px; ">
                            </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td style="padding: 10px 15px 10px 15px; text-align: center;">
                                    <pre style="tab-size: 16; font-size: 14px;"><strong style="align-content: center">{{ $returnPurchases->document_type_description }}</strong>         No.{{$document_number}}</pre>
                                </td>
                            </tr>
                            <tr>
            ...
            <tr>
                    ...
                    <div style="position: absolute; width: 50%; padding-top: 7px; padding-bottom: 7px">
                        ...
                        <table class="full-width">
                            ...
                            @foreach($returnPurchases->payments as $pago)
                                ...
                                <tr style="background: #f7f7f5;">
                                    <td style="text-align: start; padding-left: 15px; padding-right: 15px;">{{ $pago['description'] }}</td>
                ...
                <td width="40%">
                    ...
                    @if ($returnPurchases->currency_type_id === 'USD')
                        ...
                        @endif
                            <tr>
                                <td style="padding-left: 15px; padding-right: 15px; background: #f7f7f5;"><strong>Sin Impuestos:</strong></td>
                                <td class="text-right" style="padding-left: 15px; padding-right: 15px; background: #eaeaea;">${{ number_format($document->importeSinImpuestos, 2) }}</td>
                            </tr>

                        <tr>
                            <td style="padding-left: 15px; padding-right: 15px; background: #f7f7f5;">Descuentos:</td>
                            <td class="text-right" style="padding-left: 15px; padding-right: 15px; background: #eaeaea;">${{ number_format($document->descuento, 2) }}</td>
                        </tr>
                        <!-- JOINSOFTWARE -->
                        @foreach($totales as $total)
                        <tr>
                            <td style="padding-left: 15px; padding-right: 15px; background: #f7f7f5;">IVA {{$total['tarifa']}}%:</td>
                            <td class="text-right" style="padding-left: 15px; padding-right: 15px; background: #eaeaea;">${{number_format($total['iva'],2)}}</td>
                        </tr>
                        @endforeach
                        <tr>
                        ...
                        <tr>
                            <td style="padding-left: 15px; padding-right: 15px; background: #f7f7f5;"><strong>Total:</strong></td>
