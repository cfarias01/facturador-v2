...
<factura id="comprobante" version="1.1.0">
    ...
    @foreach($document->items as $row)
        <detalle>
			...
			@if(str_contains($row->item->description,"COD:"))
			@php
				$codPos = strrpos($row->item->description,"COD:");
				$cod = substr($row->item->description,$codPos);
				$cod = trim(str_replace("COD:","",$cod));
			@endphp
			<codigoAuxiliar>{{ $cod }}</codigoAuxiliar>
			@endif
            <descripcion>{{ trim(str_replace(["COD:",$cod],"",$row->item->description)) }}</descripcion>
