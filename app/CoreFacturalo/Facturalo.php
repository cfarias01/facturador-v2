...
class Facturalo
{
    use StorageDocument, KardexTrait;
    ...
    */
    ...
    private function getQr()
    {
        ...
        */
        ...
        //$qr = $qrCode->displayPNGBase64($this->document->claveAcceso);
        //JOINSOFTWARE
    ...
    public function createPdf($document = null, $type = null, $format = null, $output = 'pdf')
    {
        ...
        $details = DetalleFacturaElectronica::where('idComporbante', $document->id)->get();

        if ($this->type == '06' || $this->document->tipoComprobante == 6) {
            $details = Destinatarios::where('id_documento', $document->idComporbante)
                ->get();

            //Log::info("Facturalo guias de remision: ".json_encode($details));
        }

        $returnSales = (object) CabeceraDocumentoElectronica::findOrFail($document->id)->getCollectionData();
        ...
        if ($base_pdf_template === 'blank' && in_array($this->document->tipoComprobante, ['09'])) {
            $pdf_margin_top = 15;
            $pdf_margin_right = 5;
            $pdf_margin_bottom = 15;
            $pdf_margin_left = 14;
        ...
        $html = $template->pdf($base_pdf_template, $this->type, $this->company, $this->document, $details, $returnSales, $format_pdf);
    ...
    /*Funcion solo para recuperar los XML del SRI */

    public function getXmlSoapSRI($claveAcceso){
        ...
        $request = new AuthSri();
        $authSRI = $request->send($url, $claveAcceso);
    ...
    public function validateDocumentSRI()
    {
        ...
        if ($authSRI != '') {
            ...
            if ($estado == 'RECHAZADA') {
                ...
                if ($this->document->document_type_id === '07') {

                    $tipodoc = 'note';
                    $this->doc_type = '07';
                }
                if ($this->document->document_type_id === '05') {

                    $tipodoc = 'debit';
                    $this->doc_type = '05';
                }
                if ($this->document->document_type_id === '06') {
