...
class UploadFileHelper
{
    ...
    public static function checkIfValidFile($filename, $temp_path, $is_image = true, $mimes = null, $allowed_file_types = null)
    {
        ...
        if (!in_array(mime_content_type($temp_path), $allowed_file_types, true)) self::notAllowedFile('Tipo de archivo MIME no permitido');
    ...
    public static function imageCanBeProcessed($temp_path)
    {
        try {
            ...
            if ($mime === 'image/svg+xml') {
                // SVG files are not supported by Intervention Image, skip processing
                return [
                    'success' => true,
                ];
            }
    ...
    {
        return ['image/jpg', 'image/jpeg', 'image/png', 'image/gif', 'image/svg', 'image/svg+xml'];
    }
